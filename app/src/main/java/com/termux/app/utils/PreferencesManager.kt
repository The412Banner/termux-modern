...
class PreferencesManager(private val context: Context) {
    ...
    val colorScheme: Flow<ColorScheme> = context.dataStore.data.map { prefs ->
        try { ColorScheme.valueOf(prefs[COLOR_SCHEME] ?: ColorScheme.DRACULA.name) }
        catch (e: Exception) { ColorScheme.DRACULA }
    }
    ...
    val bellMode: Flow<BellMode>          = context.dataStore.data.map { prefs ->
        try { BellMode.valueOf(prefs[BELL_MODE] ?: BellMode.VIBRATE.name) }
        catch (e: Exception) { BellMode.VIBRATE }
    }
